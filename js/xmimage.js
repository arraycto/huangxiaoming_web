		...
		if(data.Status == 1){
			...
			for (var i = 0; i < data.Result.length; i++) {
				for (var x = 0; x < data.Result[i].Book.length; x++) {
					smlist += "<div class='col-xs-12 col-sm-6 col-lg-4 isotope-item' data-filter="+data.Result[i]['ClassName']+"><ul class='lightgallery-album'><li><div class='thumbnail-type-3' id="+data.Result[i].Book[x]['Tag']+"><a class='thumbnail-img'><img src="+url+data.Result[i].Book[x]['TagCover']+" class='imghei' style='width:100%;'><div class='caption'><div class='caption-inner'><div class='caption-title'>"+data.Result[i].Book[x]['Tag']+"</div><hr/><div class='caption-date'>"+data.Result[i].Book[x]['Count']+"张</div></div></div></a></div></li></ul></div>"
				}
