...
$document.ready(function() {
	...
	function initOwlCarousel(c) {
		var aliaces = ["-", "-xs-", "-sm-", "-md-", "-lg-", "-xl-"],
			values = [0, 520, 768, 992, 1200, 1600],
			responsive = {},
			j, k;
		...
		c.on("translate.owl.carousel", function(event) {
			$(this).find('.jp-jplayer').jPlayer("pause");
		});
		c.on("initialized.owl.carousel", function(event) {
			var jplayerVideo = $(this).find('.jp-video-init');
			$.each(jplayerVideo, initJplayerVideo);
			$(this).find('[data-lightgallery="group"]').lightGallery({
				thumbnail: true,
				thumbMargin: 20,
				subHtmlSelectorRelative: true,
				addClass: $(this).find('[data-lightgallery="group"]').attr('data-lg-skin') || "lg-skin-2",
				selector: "[data-lightgallery='group-item']"
			});
		});
		return c.owlCarousel({
	...
	// 		newMessage: "请输入正确的邮箱地址"
	// 	}, {
	// 		type: regula.Constraint.Numeric,
	// 		newMessage: "请输入您的真实姓名"
	// 	}, {
...
// }
	...
	function initJplayerVideo(index, item) {
		var $item = $(item);
	...
	if(plugins.jPlayerVideo.length) {
		var filteredJplayer = plugins.jPlayerVideo.filter(function() {
			return $(this).parent().is(":not(.owl-item-inner)");
		});
		$.each(filteredJplayer, initJplayerVideo);
		...
		for(i = 0; i < plugins.swiper.length; i++) {
			...
			swiperSlide.end().find("[data-caption-animate]").addClass("not-animated").end().swiper({
				...
				onInit: function(swiper) {
					...
					var jplayerVideo = $(swiper.wrapper).find('.jp-video-init');
					$.each(jplayerVideo, initJplayerVideo);
	...
	if(plugins.lightGallery.length && !isNoviBuilder) {
		var filteredGallery = plugins.lightGallery.filter(function(index) {
			if($(this).parents('.owl-carousel').length) {
				return false;
			}
			return true;
		});
		filteredGallery.lightGallery({
			mode: 'lg-fade',
			thumbnail: true,
			thumbMargin: 20,
			subHtmlSelectorRelative: true,
			addClass: $(filteredGallery).attr('data-lg-skin') || "lg-skin-2",
			selector: "[data-lightgallery='group-item']",
			videojs: true
		}).on('onBeforeSlide.lg', function(e) {
			$('.lg-outer').css('pointer-events', 'none');
		}).on('onAfterSlide.lg', function(e) {
			$('.lg-outer').css('pointer-events', 'all');
		});
	}
	if(plugins.lightGalleryItem.length && !isNoviBuilder) {
		var filteredGallery = plugins.lightGalleryItem.filter(function(index) {
			if($(this).parents('.owl-carousel').length) {
				return false;
			}
			return true;
	...
	if(plugins.bootstrapTabs.length) {
		...
		for(i = 0; i < plugins.bootstrapTabs.length; i++) {
			...
			if(bootstrapTabsItem.find('.owl-carousel').length) {
				var carouselObj = bootstrapTabsItem.find('.tab-content .tab-pane .owl-carousel');
				for(i = 0; i < carouselObj.length; i++) {
					initOwlCarousel($(carouselObj[i]));
				}
			}
